...
* "게임용어 탐구생활 3탄" - [로그라이크](https://namu.wiki/w/%EB%A1%9C%EA%B7%B8%EB%9D%BC%EC%9D%B4%ED%81%AC) 장르
  * 바인딩 오브 아이작 플레이 영상 (5초)
    * "게임용어 탐구생활" 3탄 "로그라이크" 란 무엇인가 30초만에 설명해 드리겠습니다
  * "Rogue" 게임에 대한 영상 (30초)
    * 최초의 던전 탐색형 RPG 인 "Rogue" 라는 게임이며
    * 1980년 당시에는 지금과 같은 퍼스널 컴퓨터가 없었고,
    * 모니터와 키보드를  통해 문자를 전달하는 방법이 유일했기 때문에
  ...
  * 게임의 특징 - 퍼머데스
    * 이 게임은 게임을 끌 때 세이브 되고, 
    * 게임을 다시 켜면 이전 세이브 파일로 부터 불러온 뒤
    * 세이브 파일이 삭제되는 방식이라 임의 세이브가 불가능했고,
    * 이와 같이 결과를 무를 수 없는' 시스템은 이후
    *  퍼머데스(Permanent Death) 즉 영구적인 죽음으로 알려져 있습니다
  * 게임의 목적 - 반지를 찾고 승천
    * 주인공이 수십층의 지하동굴방을 탐험하며 아이템을 활용해 몬스터를 처치하며,
    * 가장 아래층인 22층 이하에서 발견할 수 있는 "Amulet of Yender" 라는
    * 부적을 착용하고 승천을 하는 것이 최종 목표.
    * 죽거나 승천하면 그때까지 기록한 경험치와 층 수가 표시되고,
    * 최고점수 기록 리더보드 랭킹이 표시되고 이것이 사용자간 경쟁요소입니다
  * 바인딩 오브 아이작 플레이
    * 
